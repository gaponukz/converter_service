...
class PrepareFilesBeforeConvertDecorator:
    ...
    def process(self, id: SessionId):
        for filename in os.listdir(f"{self.directory}/{id}"):
            ...
            extract_nested_zip(
                f"{self.directory}/{id}/{filename}", f"{self.directory}/{id}"
            )

        for filename in os.listdir(f"{self.directory}/{id}"):
            self.db.save(
                id, Tdata(f"{self.directory}/{id}/{filename.replace('.zip', '')}")
            )
...
def extract_nested_zip(archive_path: str, save_path: str):
    ...
    with zipfile.ZipFile(archive_path, "r") as archive:
        for file in archive.filelist:
            ...
            with archive.open(file.filename) as zf, open(
                f"{save_path}/{file.filename}", "wb"
            ) as out:
