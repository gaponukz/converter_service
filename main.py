...
from src.application.decorators.before_tdata_onvert import (
    PrepareFilesBeforeConvertDecorator,
)
...
from_session_converter = IgnoreErrorsProxy[Session, Tdata](
    ConvertAttemptDecorator(FromSessionConverter("tdatas_results"), convert_attempt),
    logger,
)
from_tdata_converter = IgnoreErrorsProxy[Tdata, Session](
    ConvertAttemptDecorator(FromTdataConverter("sessions_results"), convert_attempt),
    logger,
)
from_session_usecase = ConvertFromSessionToTdata(
    SessionStorage("input"), tdata_db, from_session_converter
)
from_session_usecase = MakeZipAfterConvertDecorator(from_session_usecase, "tdatas")
from_tdata_usecase = ConvertFromTdataToSession(
    SessionStorage("sessions"), TdataStorage("tdatas"), from_tdata_converter
)
from_tdata_usecase = PrepareFilesBeforeConvertDecorator(
    from_tdata_usecase, tdata_db, "input"
)
