...
class FromTdataConverterMock:
    def convert(self, tdata: Tdata) -> Session:
        ...
        return Session(json_path=f"{path}.json", session_path=f"{path}.session")
...
def test_usecase_without_exceptions():
    ...
    assert session_db.sessions == [
        Session(
            json_path=f"sessios/{session_id}/{i}/acc.json",
            session_path=f"sessios/{session_id}/{i}/acc.session",
        )
        for i in range(10)
    ]
